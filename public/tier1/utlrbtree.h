...
struct TreeNode_t
{
	...
	TreeNode_t(const TreeNode_t &from) :
		left(from.left),
		right(from.right),
		parent(from.parent),
		tag(from.tag),
...
public:
	...
	void SetTree(LessFunc_t lessFunc, M elements, I root, I numElements, I firstFree, int lastAlloc, Node_t* pElements)
	{
		memcpy(m_LessFunc, lessFunc, sizeof(LessFunc_t));
		memcpy(&m_Elements, &elements, sizeof(M));
		memcpy(&m_Root, &root, sizeof(I));
		memcpy(&m_NumElements, &numElements, sizeof(I));
		memcpy(&m_LastAlloc, &lastAlloc, sizeof(int));
